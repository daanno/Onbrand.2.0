-- Fix brand creation trigger to match actual brands table structure
-- The brands table has: id, name, display_name, primary_color, logo_url, created_at, updated_at
-- NOT: description
...
BEGIN
  ...
  ELSE
    ...
    -- Using correct column names: id, name, display_name (NOT description)
    INSERT INTO public.brands (id, name, display_name, created_at)
    VALUES (
      brand_slug,
      brand_slug,
      brand_display_name,
...
'Auto-creates brands from company email domains. Public email providers (Gmail, Yahoo, etc.) are assigned to generic "act" brand for testing. Fixed to use correct column names: display_name instead of description.';
